...
class _MultiHiveDashboardState extends State<MultiHiveDashboard> {
  ...
  void _initHives() {
    hives = [
      HiveData(
        ...
        weatherForecast: [
          DayForecast(dayName: 'Σήμερα', dateStr: '29/8', maxTemp: 28, minTemp: 19, maxWindSpeedKmH: 13.0, maxHumidity: 67, weatherCode: 1),
          DayForecast(dayName: 'Κυρ', dateStr: '30/8', maxTemp: 30, minTemp: 20, maxWindSpeedKmH: 22.0, maxHumidity: 75, weatherCode: 0),
          DayForecast(dayName: 'Δευ', dateStr: '31/8', maxTemp: 30, minTemp: 20, maxWindSpeedKmH: 14.0, maxHumidity: 71, weatherCode: 2),
          DayForecast(dayName: 'Τρι', dateStr: '1/9', maxTemp: 31, minTemp: 21, maxWindSpeedKmH: 32.0, maxHumidity: 75, weatherCode: 3),
          DayForecast(dayName: 'Τετ', dateStr: '2/9', maxTemp: 31, minTemp: 22, maxWindSpeedKmH: 12.0, maxHumidity: 77, weatherCode: 61),
        ],
        hourlyWeights48h: [
      ...
      HiveData(
        ...
        fireRiskCategoryTomorrow: 1,
        notes: [
          HiveNote(date: '20 Αυγ 2026', text: 'Τροφοδοσία με ζαχαροζύμαρο 1kg. Καλή δραστηριότητα εισόδου.'),
        ],
        weatherForecast: [
          DayForecast(dayName: 'Σήμερα', dateStr: '29/8', maxTemp: 32, minTemp: 20, maxWindSpeedKmH: 21.0, maxHumidity: 55, weatherCode: 0),
          DayForecast(dayName: 'Κυρ', dateStr: '30/8', maxTemp: 33, minTemp: 21, maxWindSpeedKmH: 12.0, maxHumidity: 52, weatherCode: 0),
          DayForecast(dayName: 'Δευ', dateStr: '31/8', maxTemp: 31, minTemp: 19, maxWindSpeedKmH: 26.0, maxHumidity: 68, weatherCode: 2),
          DayForecast(dayName: 'Τρι', dateStr: '1/9', maxTemp: 29, minTemp: 18, maxWindSpeedKmH: 18.5, maxHumidity: 74, weatherCode: 3),
          DayForecast(dayName: 'Τετ', dateStr: '2/9', maxTemp: 28, minTemp: 17, maxWindSpeedKmH: 22.0, maxHumidity: 80, weatherCode: 61),
        ],
        hourlyWeights48h: [
  ...
  Future<void> _fetchLiveOnlineData() async {
    ...
    try {
      final weatherUrl = Uri.parse(
        'https://api.open-meteo.com/v1/forecast?latitude=${hive.latitude}&longitude=${hive.longitude}&current=temperature_2m,relative_humidity_2m&hourly=relative_humidity_2m&daily=weathercode,temperature_2m_max,temperature_2m_min,windspeed_10m_max&timezone=auto',
      );
      ...
      if (weatherRes.statusCode == 200) {
        ...
        final List hourlyHumidities = hourly != null ? List.from(hourly['relative_humidity_2m']) : [];
      ...
      if (fireRiskRes.statusCode == 200) {
        ...
        if (regions != null) {
          int todayCat = regions['Ξάνθη'] ?? 2;
          setState(() {
            hive.fireRiskCategoryToday = todayCat;
            hive.fireRiskCategoryTomorrow = todayCat;
          });
        }
      }
  ...
  void _showRenameHiveDialog() {
    final controller = TextEditingController(text: activeHive.name);
    showDialog(
      context: context,
      builder: (context) {
        return AlertDialog(
          shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
          title: Text(t['rename_hive']!, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 17)),
          content: TextField(
            controller: controller,
            autofocus: true,
            decoration: InputDecoration(
                  ...
                  children: [
                    ...
                    Text(
                      ...
                      style: TextStyle(
                        ...
                        color: isRestricted ? const Color(0xFF7F1D1D) : const Color(0xFF14532D),
                      ),
                    ),
                  ],
                ),
  ...
  Widget build(BuildContext context) {
    ...
    // Seasonal check: Fire season runs from May 1st through October 31st inclusive.
    final now = DateTime.now();
    final bool isFireSeason = now.month >= 5 && now.month <= 10;
    
    // Time check: Civil protection updates daily map at 14:00 (2 PM)
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: GestureDetector(
          onLongPress: _showRenameHiveDialog,
          child: Row(
            mainAxisSize: MainAxisSize.min,
            children: [
              Flexible(
        ...
        child: ConstrainedBox(
          ...
          child: ListView(
            ...
            children: [
              ...
              // Compact 4-Metric Row with Even Spacing
              Row(
                children: [
              ...
              // 🚒 SPLIT FIRE HAZARD WIDGET (Shown only between May-Oct; shows only Today if before 14:00)
              if (isFireSeason) ...[
                Container(
                          ...
                          child: Container(
                            ...
                            child: Column(
                              children: [
                                ...
                                Text(day.dateStr, style: const TextStyle(fontSize: 9, color: Colors.grey, fontWeight: FontWeight.w600)),
                                  ...
                                  children: [
                                    if (isHighWind)
                                      SizedBox(
                                        width: 15,
                                        height: 11,
                                        child: CustomPaint(painter: WindsockPainter(isAlert: true)),
                                      )
                                    else
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final fillPaint = Paint()
      ..shader = LinearGradient(
        colors: [const Color(0xFFF59E0B).withValues(alpha: 0.35), const Color(0xFFF59E0B).withValues(alpha: 0.0)],
...
class WindsockPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    final orange = Paint()
      ..color = (isAlert ? const Color(0xFFEA580C) : const Color(0xFFF97316))
      ..style = PaintingStyle.fill;
    ...
    final line = Paint()
      ..color = (isAlert ? const Color(0xFF9A3412) : const Color(0xFF94A3B8))
      ..strokeWidth = 1.0
      ..style = PaintingStyle.stroke;
